...
function getCookie(c_name) {
	...
	var value = $.cookie(c_name);

	return value;
}

function setCookie(c_name, value, pathUrl, exdays) {

	$.cookie(c_name, value, { path: pathUrl, expires: exdays });

}

//radio.js

//radio key down event(p)
$(document).keydown(function (e) {

    if (e.keyCode == '80') {
        if ($("#jquery_jplayer_1").data("jPlayer").status.paused == true) { $("#jquery_jplayer_1").jPlayer("play"); setCookie('radioonoff', 'true+' + channelposition, '/', 10); } else { $("#jquery_jplayer_1").jPlayer("clearMedia"); setCookie('radioonoff', 'false', '/', 1); }
    }
    else if (e.keyCode == '90') { $('#telugufontcontrols,#englishfontcontrols').show(); }
    else if (e.keyCode == '88') { $('#telugufontcontrols,#englishfontcontrols').hide(); }
});

/*continious play window*/
function Radiowindow(crtlid) { $('#opennewwindowimage').hide(); setCookie("radionewwindow", "opend", 1); $("#jquery_jplayer_1").jPlayer("clearMedia"); var ctrlcontent = document.getElementById(crtlid); var scripts = document.getElementById('scripts'); var printscreen = window.open('', '', 'left=1,top=1,width=435,height=170,toolbars=no,menubar=no,location=no,scrollbars=no,resizable=no,status=yes'); printscreen.parent.document.title = "APHerald Radio"; printscreen.document.write("<style type='text/css'>.apheraldradio{width:374px;height:auto;margin-left:13px;margin-top:12px;} .marginbottom10{margin-bottom:10px}</style><div id='apheraldradio' class='apheraldradio'><a href='http://www.apherald.com' target='_blank'><img width='370px' src='/images/Fan.png' class='marginbottom10'/></a><iframe src='http://183.82.0.39:90/radio.html' width='380' frameborder='no'></ifrrame>" + ctrlcontent.innerHTML + "</div>"); printscreen.document.close(); printscreen.onbeforeunload = WindowCloseHanlder; function WindowCloseHanlder() { setCookie("radionewwindow", "notopend", 1); $('#opennewwindowimage').show(); } printscreen.focus(); setCookie('radioonoff', 'false', '/', 1); }

//var mychannels = radiochannelslist();
//radio functionality start
var channelposition = 0; var loadvariable = '0'; var controlwindow = "notopen"; var mychannels = radiochannelslist();
// loading time calling jplayer
//alert(getCookie('radioonoff'));
if (getCookie('radioonoff') != "false" && getCookie('radioonoff') != "undefined" && getCookie('radioonoff') != null) { loadvariable = getCookie('radioonoff').split("+")[1]; } mediaplayer(loadvariable);

// alert('hi2');
/*radio chunning to next channel*/$('.jp-next').click(function () { channelposition++; if (mychannels.length == channelposition) { channelposition = 0; } playfunction(channelposition); });
function playfunction(positionvariable) { $("#jquery_jplayer_1").jPlayer("clearMedia"); var nextstation = { title: mychannels[positionvariable][1], mp3: mychannels[positionvariable][0] }; $("#jquery_jplayer_1").jPlayer("setMedia", nextstation).jPlayer("play"); $('.jp-playlist ul li').text('').html("<span id='rdstationname' class='ChannelImage' style='max-width:100%;line-height:0.4' onclick='toggleSlider(\"channelsdiv\",\"channel\",\"hindichannels\")'>Radio " + mychannels[positionvariable][1] + "</span><div class='radio-arrow-down'></div>"); }
/*radio chunning to previous  channel*/$('.jp-previous').click(function () { channelposition--; if (mychannels.length == -1) { channelposition = mychannels.length - 1; } playfunction(channelposition); });
/*onclick method of each channel*/$('#channel span,#hindichannels span').click(function () {  $('#channel,#hindichannels').hide(); playfunction($(this).attr('id')); });

/*jplayer intialization*/
function mediaplayer(position) {

    $('#rdstationname').text('').text("Radio " + mychannels[position][1]); channelposition = position; var stream = { title: mychannels[position][1], mp3: mychannels[position][0] }, ready = false;
    $("#jquery_jplayer_1").jPlayer({
        ready: function (event) { ready = true; if (getCookie('radioonoff') != "false" && getCookie('radioonoff') != "undefined" && getCookie('radioonoff') != null) $(this).jPlayer("setMedia", stream).jPlayer("play"); else $(this).jPlayer("setMedia", stream); },
        play: function () { if (getCookie('radionewwindow') == "opend") setCookie('radioonoff', 'false', '/', 1); else setCookie('radioonoff', 'true+' + channelposition, '/', 10); },
        pause: function () { $(this).jPlayer("clearMedia"); setCookie('radioonoff', 'false', '/', 1); },
        error: function (event) { if (ready && event.jPlayer.error.type === $.jPlayer.error.URL_NOT_SET) { $(this).jPlayer("setMedia", stream).jPlayer("play"); } },
        swfPath: "/Scripts", supplied: "mp3", preload: "none", height: "50px", wmode: "window"
    });
}
$(document).mouseup(function (e) { $('#channel').hide(); $('#hindichannels').hide(); });
function radiochannelslist() {
    var mychannels = [
    //['http://98.226.223.128:8097/;stream.mp3', 'Kokila'],
    ['http://173.199.169.114:9012/;stream.mp3', 'mansutho emotional'],
    ['http://www.radiokhushi.com:9010/;stream.mp3', 'radiokhushi'],
    ['http://174.37.252.208:8530/;stream.mp3', 'radio9'],
    ['http://s7.voscast.com:7184/;stream.mp3', 'telugu vani'],
    //['http://184.168.74.213:7001/;stream.mp3', 'tharanga'],
    ['http://38.101.195.5:8332/;stream.mp3', 'telugu one radio'],
    ['http://38.101.195.5:9156/;stream.mp3', 'tori live fm'],
    ['http://38.101.195.5:8232/;stream.mp3', 'tori telugu'],
    //['http://94.23.0.147:8128/;stream.mp3', 'big92.7fm'],
    ['http://208.115.222.205:9998/;stream.mp3', 'radiocity'],
   // ['http://213.175.221.132:9001/;stream.mp3', 'red fm 93.5'],
   // ['http://69.162.106.5:8300/;stream.mp3', 'deccan'],
   // ['http://173.192.224.123:8559/;stream.mp3', 'Apma'],
   // ['http://radio2.desi-radio.com:80/;stream.mp3', 'desiradio'],
    ['http://38.101.195.5:8756/;stream.mp3', 'Rehman Era'],
   // ['http://38.101.195.5:9600/;stream.mp3', 'Ghantasla - Hits'],
    ['http://38.101.195.5:8656/;stream.mp3', 'SP Balu & Ilayaraja Era'],
    ['http://183.82.0.39:90/Shadow_Title.mp3', 'Shadow Title song']
    ];
    return mychannels;
}
$('#rdstationname').click(function () { $('#channel,#hindichannels').show(); });
function toggleSlider(id, leadid, hiddingvar) {
    $('#' + hiddingvar).hide();
    if ($("#" + leadid).is(":visible")) {
        ...
        $("#" + leadid).css('display', 'inline-block');
        $("#" + leadid).slideDown(800, function () {
            $("#" + leadid).animate(
                     {
